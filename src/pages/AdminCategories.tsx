			...
			<div className="space-y-4">
					...
					<div className="flex items-center justify-between gap-4">
						<div className="flex items-center gap-4">
							...
							<div>
								...
								<p className="text-2xl font-regular text-slate-900">{categories.length}</p>
							</div>
						</div>
						...
						<button
							...
							className="flex items-center justify-center gap-2 px-3 py-3 h-10 rounded-lg bg-ocean-600 text-white text-sm font-normal hover:bg-ocean-700 transition-colors flex-shrink-0"
						>
				...
				{categories.length === 0 ? (
					<div className="text-center py-12 bg-white rounded-xl border border-slate-200">
						...
						<h3 className="text-md font-regular text-slate-900 mb-1">Нет категорий</h3>
						<p className="text-sm text-slate-500">Создайте первую категорию для отзывов</p>
					</div>
				) : (
					<div className="bg-white rounded-xl border border-slate-200 divide-y divide-slate-100 overflow-hidden">
						{categories.map((category, index) => (
							<div key={category.id} className={`p-4 hover:bg-slate-50 transition-colors ${index === 0 ? 'rounded-t-[10px]' : ''
								...
								{editingId === category.id ? (
									<div className="flex items-center gap-3">
										...
										<button
											...
											className="px-3 py-2 h-10 w-24 rounded-lg bg-emerald-600 text-white text-sm font-regular hover:bg-emerald-700 disabled:opacity-50 transition-colors flex items-center justify-center"
										>
										...
										<button
											...
											className="px-3 py-2 h-10 rounded-lg border border-slate-200 text-slate-600 text-sm font-regular hover:bg-slate-50 transition-colors"
										>
											Отмена
										</button>
									</div>
									...
									<div className="flex items-center justify-between">
										<div className="flex items-center gap-3 flex-1 min-w-0">
											<div className="w-10 h-10 bg-gradient-to-br from-ocean-100 to-ocean-200 rounded-lg flex items-center justify-center flex-shrink-0">
												<span className="text-ocean-700 text-md font-regular">{index + 1}</span>
											</div>
											<div className="flex-1 min-w-0">
												<h3 className="text-md font-normal text-slate-900 truncate">{category.name}</h3>
					...
					<div className="bg-white rounded-xl border border-slate-200 shadow-lg p-6 max-w-sm mx-4">
						<div className="flex items-center gap-3 mb-4">
							...
							<div className="flex-1">
								<h3 className="text-md font-medium text-slate-900">Удалить категорию?</h3>
								<p className="text-sm text-slate-500 mt-0.5">"{deleteConfirmName}"</p>
							</div>
						</div>
